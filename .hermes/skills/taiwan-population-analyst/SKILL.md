...
4. For `status: ok`, base every number on `results`, state month, geographic level, sex, age scope, unit, and ties. For any other status, explain the returned code and available coverage or request clarification. `results: null` means unavailable, never zero. Do not retry with different conditions unless the user supplies or approves them.
